...
export function downloadExamQuestionsPDF(questions: Question[]): void {
  ...
  questions.forEach((q, idx) => {
    ...
    if (q.imageSvg) {
      ...
      doc.text('[Lihat diagram alir / ilustrasi visual pada tampilan aplikasi ujian]', 16, y);
      y += 5;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9.5);
      doc.setTextColor(0, 0, 0);
    ...
    // Pernyataan Kategori (Benar/Salah, Sesuai, Setuju)
    if (q.statements && q.statements.length > 0) {
      ...
      q.statements.forEach((st, sIdx) => {
        ...
        const stLines = doc.splitTextToSize(
          `Pernyataan ${sIdx + 1}: ${st.text}   [  ] ${optA}   [  ] ${optB}`,
          pageWidth - 36
        );
  ...
  doc.save(`Naskah_Soal_Koding_AI_Kelas_${CONFIG.KELAS}_${CONFIG.SEKOLAH.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`);
}
...
export function downloadResultsRecapPDF(results: ExamResult[]): void {
  ...
  doc.save(`Rekap_Nilai_Tes_Sumatif_Koding_AI_Kelas_${CONFIG.KELAS}.pdf`);
}
