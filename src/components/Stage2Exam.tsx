...
export const Stage2Exam: React.FC<Stage2ExamProps> = ({
  ...
  // Handler jawaban PGK Kategori (Benar / Salah, Setuju, Sesuai)
  ...
  const handleInputIsian = (textVal: string) => {
    setAnswers((prev) => ({
      ...prev,
      [currentIndex]: textVal,
    }));
  };
    ...
    shuffledQuestions.forEach((sq, idx) => {
      ...
      if (sq.type === 'pg') {
        ...
        if (statementsCorrect === statements.length) {
          ...
          totalScore += statementsCorrect / Math.max(statements.length, 1);
        }
      } else if (sq.type === 'isian') {
        // Isian singkat
        const userText = String(userAns || '').trim().toLowerCase();
        const acceptable = origQ.acceptableAnswers && origQ.acceptableAnswers.length > 0
          ? origQ.acceptableAnswers.map((a) => a.trim().toLowerCase())
          : [String(origQ.correctAnswer || '').trim().toLowerCase()];

        if (userText.length > 0 && acceptable.includes(userText)) {
          totalScore += 1;
          benarCount += 1;
        }
      }
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-xs border border-slate-200 min-h-[420px] flex flex-col justify-between">
            <div>
              ...
              <div className="mb-4 text-xs font-semibold px-3 py-1.5 rounded-lg bg-blue-50 text-blue-800 border border-blue-100 inline-block">
                ...
                {currentQ.type === 'pgk_kategori' && 'Tentukan respon penilaian untuk setiap butir pernyataan di bawah ini.'}
                {currentQ.type === 'isian' && 'Tuliskan jawaban singkat berupa kata atau istilah yang tepat pada kolom di bawah.'}
              </div>
              ...
              {/* Tampilan Opsi Jawaban: PGK Kategori (Benar / Salah, Setuju, Sesuai) */}
              {currentQ.type === 'pgk_kategori' && (
                <div className="space-y-3">
                  {(currentQ.statements || []).map((st, sIdx) => {
                    ...
                    // Tentukan label tombol berdasarkan categoryType
                    let labelTrue = 'BENAR';
                    let labelFalse = 'SALAH';
                    if (currentQ.categoryType === 'setuju_tidak_setuju') {
                      labelTrue = 'SETUJU';
                      labelFalse = 'TIDAK SETUJU';
                    } else if (currentQ.categoryType === 'sesuai_tidak_sesuai') {
                      labelTrue = 'SESUAI';
                      labelFalse = 'TIDAK SESUAI';
                    }
              ...
              {/* Tampilan Input Jawaban: Isian Singkat */}
              {currentQ.type === 'isian' && (
                <div className="space-y-3 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                  ...
                  >
                    Ketik Jawaban Singkat Anda: <span className="text-red-500">*</span>
                  </label>
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => handleInputIsian(e.target.value)}
                      placeholder="Ketik jawaban di sini (contoh: algoritma)..."
                      className="w-full px-4 py-3 text-sm sm:text-base rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-semibold text-slate-900 bg-white placeholder:font-normal placeholder:text-slate-400"
                    />
                  </div>
                  <p className="text-[11px] text-slate-500">
                    * Tuliskan istilah atau kata jawaban dengan ejaan yang tepat (huruf besar/kecil tidak dibedakan).
                  </p>
                </div>
              )}
            </div>
