          ...
          <div className="mt-10 pt-8 border-t border-slate-200 text-left max-w-2xl mx-auto">
            ...
            <div className="space-y-4">
              {questions.map((q, idx) => {
                let kunciText = '';
                if (q.type === 'pg') {
                  kunciText = `Kunci: ${q.correctAnswer}`;
                } else if (q.type === 'pgk') {
                  kunciText = `Kunci: ${(q.correctAnswer as string[])?.join(', ')}`;
                } else if (q.type === 'isian') {
                  kunciText = `Kunci: "${q.correctAnswer}"`;
                } else if (q.type === 'pgk_kategori') {
                  kunciText = 'Kunci: Sesuai Kategori';
                }
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between font-bold text-slate-700">
                      ...
                      <span className="text-blue-700">{kunciText}</span>
                    </div>
                    ...
                    {q.type === 'pgk_kategori' && q.statements && (
                      <div className="space-y-1 my-1 p-2 bg-white rounded-lg border border-slate-200">
                        {q.statements.map((st, sI) => {
                          let statusLabel = st.correctAnswer ? 'BENAR' : 'SALAH';
                          if (q.categoryType === 'setuju_tidak_setuju') {
                            statusLabel = st.correctAnswer ? 'SETUJU' : 'TIDAK SETUJU';
                          } else if (q.categoryType === 'sesuai_tidak_sesuai') {
                            statusLabel = st.correctAnswer ? 'SESUAI' : 'TIDAK SESUAI';
                          }
                          return (
                            <div key={st.id} className="flex justify-between items-center text-[11px]">
                              <span>{sI + 1}. {st.text}</span>
                              <span className={`font-bold ml-2 ${st.correctAnswer ? 'text-emerald-700' : 'text-rose-700'}`}>
                                [{statusLabel}]
                              </span>
                            </div>
                          );
