          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-100 gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500">
                {questions.filter((q) => q.type === 'pg').length} Soal PG,{' '}
                {questions.filter((q) => q.type === 'pgk').length} Soal PGK,{' '}
          ...
          <div className="space-y-3">
            {questions.map((q) => (
              ...
              >
                ...
                {q.statements && (
                  <div className="space-y-1.5 text-xs">
                    {q.statements.map((st, sIdx) => {
                      let label = st.correctAnswer ? 'BENAR' : 'SALAH';
                      if (q.categoryType === 'setuju_tidak_setuju') {
                        label = st.correctAnswer ? 'SETUJU' : 'TIDAK SETUJU';
                      } else if (q.categoryType === 'sesuai_tidak_sesuai') {
                        label = st.correctAnswer ? 'SESUAI' : 'TIDAK SESUAI';
                      }
                ...
                {/* Isian Singkat */}
                {q.type === 'isian' && (
                  <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-xs flex items-center flex-wrap gap-2">
                    ...
                    {q.acceptableAnswers && q.acceptableAnswers.length > 1 && (
                      <span className="text-slate-500 text-[11px]">
                        (Variasi diterima: {q.acceptableAnswers.join(', ')})
                      </span>
                    )}
                  </div>
                )}
