...
export const Stage1Identity: React.FC<Stage1IdentityProps> = ({
  ...
  // Generate opsi tahun untuk siswa SD
  const currentYear = new Date().getFullYear();
  const years = Array.from({ length: 12 }, (_, i) => String(currentYear - 17 + i));
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="mt-2 text-sm text-slate-600 max-w-xl mx-auto">
          Kelas {CONFIG.KELAS} • Materi Pokok: <span className="font-semibold text-slate-800">{CONFIG.MATERI}</span>.
          Silakan masukkan nama lengkap dan nomor absen Anda untuk memulai ujian online.
        </p>
      </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-slate-900 text-white rounded-2xl p-5 shadow-sm">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-1.5">
              ...
              Komposisi & Format Soal ({CONFIG.MATERI})
            </h4>
            <div className="space-y-2 text-xs">
              ...
              <div className="flex items-center justify-between bg-slate-800/80 p-2.5 rounded-lg border border-slate-700">
                <span>3. PGK Kategori (Benar/Salah, Sesuai, Setuju)</span>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 text-[11px] text-slate-400 space-y-1.5">
              <p className="flex items-start gap-1.5">
                ...
                <span>Seluruh soal berfokus pada materi <strong>{CONFIG.MATERI}</strong> ({CONFIG.MATA_PELAJARAN} Kelas {CONFIG.KELAS}).</span>
              </p>
              <p className="flex items-start gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-blue-400 shrink-0 mt-0.5" />
                <span>Soal dan opsi pilihan diacak otomatis setiap kali tes baru dimulai.</span>
              </p>
              <p className="flex items-start gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-blue-400 shrink-0 mt-0.5" />
                <span>Seluruh 35 soal wajib dijawab sebelum jawaban dapat dikirimkan.</span>
              </p>
            </div>
          </div>
