...
export const QuestionEditorModal: React.FC<QuestionEditorModalProps> = ({
  ...
  const [acceptableText, setAcceptableText] = useState<string>(
    question.acceptableAnswers ? question.acceptableAnswers.join(', ') : ''
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (question.type === 'pg' || question.type === 'pgk') {
      finalCorrectAnswer = correctAnswer;
    } else if (question.type === 'isian') {
      finalCorrectAnswer = isianAnswer.trim().toLowerCase();
      const splitted = acceptableText
        .split(',')
        .map((s) => s.trim().toLowerCase())
        .filter((s) => s.length > 0);
      if (!splitted.includes(finalCorrectAnswer)) {
        splitted.unshift(finalCorrectAnswer);
      }
      finalAcceptable = splitted;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 relative max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto py-4 space-y-4 pr-1 text-xs sm:text-sm">
          ...
          {/* Input Kunci Jawaban Isian Singkat */}
          {question.type === 'isian' && (
            <div className="space-y-3 p-3.5 bg-slate-50 rounded-xl border border-slate-200">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Kunci Jawaban Utama (Isian Singkat) <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={isianAnswer}
                  ...
                  className="w-full px-3 py-2 rounded-lg border border-slate-300 text-xs font-bold text-slate-900 bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  required
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Variasi Jawaban Tambahan yang Diterima (Opsional, pisahkan dengan koma)
                </label>
                <input
                  type="text"
                  value={acceptableText}
                  onChange={(e) => setAcceptableText(e.target.value)}
                  placeholder="Contoh: algoritma, algoritme, algorithm"
                  className="w-full px-3 py-2 rounded-lg border border-slate-300 text-xs font-medium text-slate-800 bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                />
                <p className="text-[11px] text-slate-500 mt-1">
                  Siswa yang mengetik salah satu kata di atas akan tetap dihitung benar secara otomatis (case-insensitive).
                </p>
              </div>
            </div>
          )}
